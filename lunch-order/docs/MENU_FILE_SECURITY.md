...
### 1. 厳格なファイル参照チェック（実装済み）

**`data/menu-schedule.ts`以外のファイルを絶対に参照しない**

- `getCurrentMenu()`関数は`data/menu-schedule.ts`の`CURRENT_MENU`のみを参照
- ファイルが見つからない場合、またはメニューが空の場合は明確なエラーを投げる
- **フォールバックやデフォルト値は一切使用しない**

### 2. エラーハンドリング（実装済み）

**メニューファイルが見つからない場合、必ずエラーを表示**

- APIエンドポイント: HTTP 500エラーと明確なエラーメッセージを返す
- フロントエンド: コンソールエラーとトースト通知でエラーを表示
- エラーメッセージには「`data/menu-schedule.ts`が存在しない」ことが明記される
...
`package.json`の`prebuild`スクリプトで、ビルド前に自動チェックが実行されます。
...
**`data/menu-schedule.ts`以外のファイルを参照しない**
...
**ビルド前にメニューファイルがGit管理下にあることを確認**

- `npm run build`実行前に自動的にチェック
- チェックに失敗した場合、ビルドを中断

import { NextResponse } from 'next/server'
import { DRINKS } from '../../../../data/menu-schedule'
import { getEffectiveMenus } from '@/lib/menu-source'

export async function GET(
  request: Request,
  { params }: { params: Promise<{ weekday: string }> }
) {
  try {
    const { weekday } = await params
    
    // 現在のメニューを取得
    const menus = await getEffectiveMenus()
    
    // 曜日の検証
    const validWeekdays = Object.keys(menus)
    if (!validWeekdays.includes(weekday)) {
      return NextResponse.json(
        { 
          success: false, 
          error: "Invalid weekday. Valid weekdays are: " + validWeekdays.join(", ") 
        },
        { status: 400 }
      )
    }
    
    // 指定された曜日のメニューを取得
    // ⚠️ 重要: data/menu-schedule.ts の CURRENT_MENU のみを参照
    // メニューが見つからない場合はエラー
    const menu = menus[weekday as keyof typeof menus]
    if (!menu || menu.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: `❌ CRITICAL ERROR: Menu data for ${weekday} is not found in data/menu-schedule.ts. ` +
                 "The file data/menu-schedule.ts must exist and contain valid menu data. " +
                 "Do not use any fallback or default menu data."
        },
        {
          status: 500,
          headers: {
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
            'Access-Control-Allow-Headers': 'Content-Type, Authorization',
          },
        }
      )
    }
    
    // 全ての飲み物をフラット化
    const allDrinks = [
      ...DRINKS.hot,
      ...DRINKS.cold,
      ...DRINKS.other
    ]
    
    const response = {
      success: true,
      data: {
        weekday: weekday,
        dishes: menu,
        drinks: allDrinks,
        drinksByCategory: DRINKS
      }
    }
    
    return NextResponse.json(response, {
      headers: {
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
        'Access-Control-Allow-Headers': 'Content-Type, Authorization',
      },
    })
  } catch (error) {
    const errorMessage = error instanceof Error 
      ? error.message 
      : "❌ CRITICAL ERROR: Failed to load menu data from data/menu-schedule.ts. " +
        "The file data/menu-schedule.ts must exist and contain valid menu data. " +
        "Do not use any fallback or default menu data."
    
    console.error("Menu API Error:", errorMessage, error)
    
    return NextResponse.json(
      { 
        success: false, 
        error: errorMessage
      },
      { 
        status: 500,
        headers: {
          'Access-Control-Allow-Origin': '*',
          'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
          'Access-Control-Allow-Headers': 'Content-Type, Authorization',
        },
      }
    )
  }
}

export async function OPTIONS() {
  return new NextResponse(null, {
    status: 200,
    headers: {
      'Access-Control-Allow-Origin': '*',
      'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
      'Access-Control-Allow-Headers': 'Content-Type, Authorization',
    },
  })
} 
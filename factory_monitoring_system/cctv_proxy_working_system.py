...
class CCTVProxyWorkingSystem:
    def __init__(self):
        ...
        # 試行するURL構造
        self.test_urls = [
            # 中間ルーター経由でのCCTV映像アクセス
            f"http://{self.router_ip}/{self.cctv_ip}/cgi-bin/guest/Video.cgi?media=MJPEG",
            f"http://{self.router_ip}/cgi-bin/guest/Video.cgi?media=MJPEG&ip={self.cctv_ip}",
            f"http://{self.router_ip}/video/{self.cctv_ip}/cgi-bin/guest/Video.cgi?media=MJPEG",
            f"http://{self.router_ip}/proxy/{self.cctv_ip}/cgi-bin/guest/Video.cgi?media=MJPEG",
            # 可能性のあるポート転送
            f"http://{self.router_ip}:8080/cgi-bin/guest/Video.cgi?media=MJPEG",
            f"http://{self.router_ip}:8000/cgi-bin/guest/Video.cgi?media=MJPEG",
        ]
    ...
    def discover_cctv_video_path(self):
        """CCTV映像パスを自動発見"""
        print("🔍 CCTV映像パス自動発見開始...")
        self.debug_info = []
        
        # 1. 中間ルーター経由でCCTV管理画面にアクセス
        try:
            ...
            if response.status_code == 200:
                ...
                # よくある映像URLパターンを検索
                video_patterns = [
                    r'/cgi-bin/guest/Video\.cgi\?media=MJPEG',
                    r'/video\.cgi\?media=MJPEG',
                    r'/mjpeg\.cgi',
                    r'/video/mjpeg',
                    r'media=MJPEG',
                ]
                ...
                if found_paths:
                    self.debug_info.append(f"✅ 発見された映像パス: {found_paths}")
                    print(f"✅ 発見された映像パス: {found_paths}")
                else:
                    self.debug_info.append("⚠️ HTMLから映像パスが見つかりません")
                    print("⚠️ HTMLから映像パスが見つかりません")
            ...
            print(f"❌ CCTV管理画面アクセスエラー: {e}")
            self.debug_info.append(f"❌ CCTV管理画面エラー: {str(e)[:50]}")
                ...
                if response.status_code == 200:
                    if 'multipart' in content_type.lower() or 'image' in content_type.lower():
                        ...
                        try:
                            ...
                            # 最初の1024バイトを取得
                            chunk = next(get_response.iter_content(chunk_size=1024), b'')
                            get_response.close()
                            
                            # JPEG画像の開始バイトをチェック
                            if chunk.startswith(b'\xff\xd8') or b'--boundary' in chunk:
    ...
    def test_cctv_connections(self):
        ...
        # 1. CCTV映像パス自動発見
        self.discover_cctv_video_path()
        
        # 2. 映像URL群テスト
        return self.test_video_urls()
...
# グローバルインスタンス
cctv_system = CCTVProxyWorkingSystem()
...
def index():
    ...
    <html>
    <head>
        <title>🏭 KIRII CCTV-YOLO (Proxy Working)</title>
        <meta charset="UTF-8">
        <style>
    ...
    <body>
        <div class="container">
            <h1>🏭 KIRII CCTV-YOLO (Proxy Working)</h1>
            
            <div class="controls">
                <button class="btn btn-info" onclick="testConnection()">🔍 映像パス発見</button>
            ...
            <div class="stats">
                ...
                <div class="stat-card">
                    <div class="stat-number">Proxy Mode</div>
                    <div>中間ルーター経由</div>
                </div>
            </div>
            ...
            <div class="debug-panel">
                <h3>🔍 映像パス発見ログ</h3>
                <div id="debugInfo">
                    <div style="color: #ccc; text-align: center;">映像パス発見を実行してください</div>
                </div>
            </div>
        ...
        <script>
            ...
            function testConnection() {
                updateStatus('🔍 映像パス発見中...', 'info');
                
                fetch('/test_connection', {method: 'POST'})
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        updateStatus('✅ 映像パス発見成功: ' + data.method, 'success');
                        updateDebugInfo(data.debug_info || []);
                    } else {
                        updateStatus('❌ 映像パス発見失敗: ' + data.error, 'error');
                        updateDebugInfo(data.debug_info || []);
                    }
                });
            }
...
if __name__ == '__main__':
    print("🏭 KIRII CCTV-YOLO監視システム (Proxy Working)")
    print("📡 中間ルーター: 192.168.0.115")
    print("📺 CCTV: 192.168.1.10")
    print("🔍 映像パス自動発見機能付き")
    print("🌐 アクセス: http://localhost:5007")

...
class CCTVYOLOSystem:
    def __init__(self):
        # CCTV設定
        self.cctv_url = "http://192.168.1.10/cgi-bin/guest/Video.cgi?media=MJPEG"
        self.username = "admin"
        self.password = "admin"
                        ...
                        while True:
                            ...
                            if start != -1 and end != -1 and end > start:
                                ...
                                if frame is not None:
                                    ...
                                    frame_count += 1
                                    if frame_count % 30 == 0:  # 30フレームごとにログ
...
def index():
    ...
    <html>
    <head>
        <title>🏭 KIRII CCTV-YOLO System</title>
        <meta charset="UTF-8">
        <style>
    ...
    <body>
        <div class="container">
            ...
            <div class="stats">
                ...
                <div class="stat-card">
                    <div class="stat-number">192.168.1.10</div>
                    <div>CCTV IP</div>
                </div>
            </div>
            ...
            function startStream() {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        isStreaming = true;
                        updateInterval = setInterval(updateFrame, 200); // 5 FPS更新
                    } else {
...
def stop_stream():
    """ストリーム停止"""
    cctv_yolo.stop_stream()
    return {'success': True}

@app.route('/get_frame')
def get_frame():
    """フレーム取得"""
    if cctv_yolo.current_frame:
        return {
            'success': True, 
            'frame': cctv_yolo.current_frame,
            'detections': cctv_yolo.detection_results
        }
    else:
        return {'success': False}

if __name__ == '__main__':
    print("🏭 KIRII CCTV-YOLO監視システム起動中...")
    print("📺 CCTV: http://192.168.1.10")
    print("🤖 YOLO11: 物体検出有効")
    print("🌐 アクセス: http://localhost:5005")

...
const supabaseAnonKey =
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || process.env.anon

// 創建一個標誌來表示我們是否處於預覽/演示模式
const isPreviewMode = !supabaseUrl || !supabaseAnonKey

// 如果處於預覽模式，只在控制台顯示一次警告
if (isPreviewMode && typeof window !== "undefined") {
  console.warn(
    "Supabase 環境變量未設置。應用程序正在預覽模式下運行，Supabase 功能將不可用。\n" +
      "要啟用完整功能，請設置 NEXT_PUBLIC_SUPABASE_URL 和 NEXT_PUBLIC_SUPABASE_ANON_KEY 環境變量。",
  )
}
